...
public class DbFriendshipDao implements FriendshipDao {
    ...
    public List<User> friendsOf(int userId) {
        return jdbc.query(
                "SELECT u.* FROM friendships f JOIN users u ON u.id = f.friend_id WHERE f.user_id = ? ORDER BY u.id",
                MAPPER,
                userId
        );
    }
