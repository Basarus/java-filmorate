...
public class GenreDbStorage implements GenreStorage {
    ...
    public List<ru.practicum.filmorate.model.Genre> findAll() {
        return jdbc.query("SELECT * FROM genres ORDER BY id", G);
    }

    @Override
    public Optional<ru.practicum.filmorate.model.Genre> findById(int id) {
        return jdbc.query("SELECT * FROM genres WHERE id = ?", G, id).stream().findFirst();
    }

    @Override
    public boolean exists(int id) {
        Integer count = jdbc.queryForObject("SELECT COUNT(*) FROM genres WHERE id = ?", Integer.class, id);
